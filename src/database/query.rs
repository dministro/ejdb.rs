...
impl Query {
    ...
    fn modify_document_at_key<K, F1, F2, V>(
        mut self,
        key: K,
        value: V,
        on_document: F1,
        ...
        V: Into<Bson>,
    {
        self.modify_document_at_key(
            ...
            |q, k, (s, v)| {
                q.insert(k.into(), bson! { s.into() => (v.into()) });
            },
        )
    }
...
impl FieldConstraint {
    ...
    pub fn field<S: Into<String>>(self, name: S) -> FieldConstraint {
        FieldConstraint(
            name.into().into(),
            FieldConstraintData::Child(Box::new(self)),
        )
    }
...
impl Q {
    ...
    #[inline(always)]
    pub fn and<I>(self, queries: I) -> Query
    where
        I: IntoIterator,
        I::Item: Into<Document>,
    {
    ...
    #[inline(always)]
    pub fn or<I>(self, queries: I) -> Query
    where
        I: IntoIterator,
        I::Item: Into<Document>,
    {
...
mod tests {
    ...
    fn test_and() {
        let q = Q
            .and(vec![Q.field("a").eq(1), Q.field("b").eq("c")])
            .into_bson();
        assert_eq!(
            q,
            bson! {
    ...
    fn test_add_to_set_all() {
        let q = Q
            .add_to_set_all("tag", vec!["tag 1", "tag 2", "tag 3"])
            .into_bson();
        assert_eq!(
            q,
            bson! {
    ...
    #[test]
    fn test_field_gt_lt() {
        let q = Q.field("x").gt(0.1).field("x").lt(9.9).into_bson();
        assert_eq!(
            q,
            bson! {
                "x" => {
    ...
    fn test_field_gte_lte() {
        let q = Q.field("y").gte(1).field("y").lte(99).into_bson();
        assert_eq!(
            q,
            bson! {
                "y" => {
    ...
    fn test_contained_in() {
        let q = Q
            .field("x")
            .contained_in(vec![1, 2, 3])
            .field("y")
            .not_contained_in(vec![7, 8, 9])
            .into_bson();
        assert_eq!(
            q,
            bson! {
    ...
    fn test_case_insensitive() {
        let q = Q
            ...
            .field("err")
            .case_insensitive()
            .contained_in(vec!["whatever", "pfff"])
            .into_bson();
        assert_eq!(
            q,
            bson! {
    ...
    fn test_strand_stror() {
        let q = Q
            .field("name")
            .str_and(["me", "xyzzy", "wab"].iter().cloned())
            .field("title")
            .str_or(["foo", "bar", "baz"].iter().cloned())
            .into_bson();
        assert_eq!(
            q,
            bson! {
